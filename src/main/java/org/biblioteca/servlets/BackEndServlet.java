    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            if (currentUser.getIsAdmin()) {
                ...
                if ((prestati == null) || (prestati.size() == 0)) {
                    ...
                    session.setAttribute("statusMsg", ok ? "Hai cancellato: " + daCancellare.getAutore() + " - " + daCancellare.getTitolo() :
                            "Errore nella cancellazione de libro  " + daCancellare.getAutore() + " - " + daCancellare.getTitolo());
                    logger.logInfo(this.currentUser.getUsername() + " ha cancellato il libro " + daCancellare.getAutore() + " - " + daCancellare.getTitolo());
                } else {
                    session.setAttribute("statusMsg", "Impossibile cancellare il libro  " + daCancellare.getAutore() + " - " + daCancellare.getTitolo() + " perché è in prestito");
                    logger.logInfo(this.currentUser.getUsername() + " non può cancellare il libro  " + daCancellare.getAutore() + " - " + daCancellare.getTitolo() + " perché è in prestito ");
                }
                resp.sendRedirect(contextPath + PATH_WEBAPP_SERVLET + "/menu/listaLibri.html");
        ...
        if (requestUri.endsWith("/eliminaUtente")) {
            if (currentUser.getIsAdmin()) {
                ...
                resp.sendRedirect(contextPath + PATH_WEBAPP_SERVLET + "/menu/listaUtenti.html");
                logger.logInfo(this.currentUser.getUsername() + " ha eliminato l'utente " + daCancellare.getUsername());
            } else resp.sendRedirect(contextPath + PATH_WEBAPP_SERVLET + "/menu/userpage.html");
        }
